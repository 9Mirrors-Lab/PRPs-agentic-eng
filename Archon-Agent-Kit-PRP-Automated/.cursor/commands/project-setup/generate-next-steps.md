# Generate Next Steps with Archon Context (Cursor Version)

## Purpose
After running project initialization, automatically generate intelligent next steps with plain English context for each command. This command leverages Archon MCP knowledge to provide project-specific guidance and eliminate the need to manually figure out what to write for each command.
...
**Run this command immediately after project initialization** to get intelligent guidance on what to do next.
...
```bash
# Generate next steps after project initialization
1. Open Cursor command palette
2. Type: "Generate next steps for project development"
3. Review the intelligent suggestions
4. Execute suggested commands with confidence

# Generate next steps for specific feature area
1. Focus on backend API development
2. Get tailored suggestions for your tech stack
3. Follow the recommended development sequence
```
...
4. **Repeat the process** - Use the command again after completing each step

## Error Handling

- **No Archon connection** - Clear instructions for MCP setup
- **Missing project context** - Guidance to run initialization first
...
This command transforms the guesswork of "what should I do next?" into intelligent, context-aware guidance that leverages your project's specific requirements and Archon's accumulated knowledge, optimized for Cursor's AI-powered development environment.
